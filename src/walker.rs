...
unsafe extern "C" fn rewrite_walker(
    ...
    if is_a(node, pg_sys::NodeTag::T_RangeTblEntry) {
        ...
        // name `pullup_replace_vars_callback` on each Range_Table_Entry
        // variable and use the original attribute number (attnum/resno) in
        // order to find the attribute.
        // Since we replace the original table with a masking subquery, there may
        // be some inconsistencies when the original tables contains dropped
        // columns.
        ...
        let target_list =
            PgList::<pg_sys::TargetEntry>::from_pg(
                (*msq_query).targetList
            );

        for target_ptr in target_list.iter_ptr() {
            let mut target =
                PgBox::<pg_sys::TargetEntry>::from_pg(
                    target_ptr as *mut pg_sys::TargetEntry
                );
