...
BEGIN
  ...
  -- ADD NEW TABLE HERE
  EXECUTE 'COPY @extschema@.city FROM       '
          || quote_literal(datapath ||'/city.csv');
  EXECUTE 'COPY @extschema@.company FROM    '
          || quote_literal(datapath ||'/company.csv');
  EXECUTE 'COPY @extschema@.email FROM      '
          || quote_literal(datapath ||'/email.csv');
  EXECUTE 'COPY @extschema@.first_name FROM '
          || quote_literal(datapath ||'/first_name.csv');
  EXECUTE 'COPY @extschema@.iban FROM       '
          || quote_literal(datapath ||'/iban.csv');
  EXECUTE 'COPY @extschema@.last_name FROM  '
          || quote_literal(datapath ||'/last_name.csv');
  EXECUTE 'COPY @extschema@.siret FROM      '
...
CREATE OR REPLACE FUNCTION @extschema@.anonymize_table(
  tablename REGCLASS
)
RETURNS BOOLEAN AS
$func$
...
RETURNS BOOLEAN AS
$func$
  SELECT @extschema@.start_dynamic_masking( sourceschema,
                                            maskschema,
                                            autoload
                                          );
$func$
...
CREATE OR REPLACE FUNCTION @extschema@.get_copy_statement(
  relid OID
)
RETURNS TEXT AS
$func$
